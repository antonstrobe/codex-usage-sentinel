...
$sourceFiles=@('README.md','AGENTS.md','build.ps1','start.ps1','test.ps1','verify-release.ps1','.gitignore','.gitattributes','.github\workflows\build.yml','docs\SETUP_WITH_CODEX.md','docs\SECOND_COMPUTER.md')
...
$sourceFiles+=@('docs\SERVER_BOT.md')
$sourceFiles+=@(Get-ChildItem -LiteralPath (Join-Path $PSScriptRoot 'server') -File | Where-Object {$_.Name -notin @('config.json','state.json')} | ForEach-Object {'server\'+$_.Name})
$releaseFiles=@('dist\CodexUsageSentinel.exe','dist\SHA256SUMS.txt')
...
if(!$SourcesOnly){foreach($line in [IO.File]::ReadAllLines((Join-Path $PSScriptRoot 'dist\SHA256SUMS.txt'))) {
    if($line -notmatch '^([a-f0-9]{64})  (CodexUsageSentinel\.exe)$'){throw 'Unexpected checksum entry.'}
